            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              {isPlace ? (
                <>
                  ...
                  <div style={{ gridColumn: "1 / -1" }}><label style={label}>Local take (EN) <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, color: P.inkSoft }}>· auto-syncs with Spanish</span></label><textarea rows={2} style={{ ...field, resize: "vertical" }} value={editing.local_take || ""} onChange={(e) => upd("local_take", e.target.value)}
                    onBlur={() => autoTranslate("local_take", "local_take_es", "es")} placeholder="The opinionated local's take" /></div>
                  <div style={{ gridColumn: "1 / -1" }}><label style={label}>Local take (ES) <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, color: P.inkSoft }}>· auto-syncs with English</span></label><textarea rows={2} style={{ ...field, resize: "vertical" }} value={editing.local_take_es || ""} onChange={(e) => upd("local_take_es", e.target.value)}
                    onBlur={() => autoTranslate("local_take_es", "local_take", "en")} placeholder="La opinión del local" /></div>
                  <div style={{ gridColumn: "1 / -1" }}><label style={label}>Internal caveat <span style={{ fontWeight: 400, textTransform: "none", letterSpacing: 0, color: P.coral }}>· Internal note (never shown publicly)</span></label><textarea rows={2} style={{ ...field, resize: "vertical" }} value={editing.caveat_internal || ""} onChange={(e) => upd("caveat_internal", e.target.value)} placeholder="What to skip / who it's not for — stays private" /></div>
                  <div><label style={label}>Vibe</label><div style={{ display: "flex", flexWrap: "wrap", gap: 8, paddingTop: 4 }}>{["romantic", "buzzy", "quiet", "cozy", "lively", "upscale", "casual", "work-friendly"].map((k) => <button key={k} onClick={() => toggle("vibe", k)} style={chip((editing.vibe || []).includes(k), P.coral)}>{k}</button>)}</div></div>
                  <div><label style={label}>Occasion</label><div style={{ display: "flex", flexWrap: "wrap", gap: 8, paddingTop: 4 }}>{["date", "celebration", "solo", "kids", "groups", "business"].map((k) => <button key={k} onClick={() => toggle("occasion", k)} style={chip((editing.occasion || []).includes(k), P.navy)}>{k}</button>)}</div></div>
