...
export default function Admin() {
  ...
  const toggleAud = (a) => setDraft((d) => {
    const set = new Set(d.audience || []);
    set.has(a) ? set.delete(a) : set.add(a);
    return { ...d, audience: [...set] };
  });

  async function normalize() {
    setBusy(true); setMsg(null); setDraft(null);
    try {
      const r = await fetch("/api/normalize", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: raw, password: pw }) });
      const j = await r.json();
      if (!r.ok) { setMsg({ type: "err", text: j.error || "Failed" }); }
      else setDraft(j.event);
  ...
  async function publish() {
    ...
    try {
      const r = await fetch("/api/publish", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ event: draft, password: pw, discovered_via: source || "paste" }) });
      const j = await r.json();
      if (!r.ok) { setMsg({ type: "err", text: j.error || "Failed" }); }
    ...
    <div style={{ minHeight: "100vh", background: P.plaster, color: P.ink, fontFamily: "system-ui, sans-serif" }}>
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "24px 18px 60px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
          <h1 style={{ fontSize: 22, margin: 0 }}>Add an event</h1>
        ...
        <p style={{ fontSize: 13.5, color: P.inkSoft, marginTop: 4 }}>Paste a Facebook post, newsletter, or flyer text. Claude turns it into a clean bilingual event — review, then publish.</p>
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <textarea value={raw} onChange={(e) => setRaw(e.target.value)} rows={8} placeholder="Paste the event text here…" style={{ ...field, resize: "vertical", lineHeight: 1.5 }} />
        </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
              ...
              <div>
                ...
                <div style={{ display: "flex", gap: 8, paddingTop: 4 }}>
                  {AUD.map(([k, l]) => (
                    <button key={k} onClick={() => toggleAud(k)}
                      style={{ padding: "7px 13px", borderRadius: 999, fontSize: 13, fontWeight: 600, cursor: "pointer",
                        border: `1px solid ${(draft.audience || []).includes(k) ? P.cobalt : P.line}`,
                        background: (draft.audience || []).includes(k) ? P.cobalt : "#fff", color: (draft.audience || []).includes(k) ? "#fff" : P.inkSoft }}>
                      {l}
                    </button>
                  ))}
                </div>
